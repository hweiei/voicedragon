import { GAME_TITLE, ITEMS, RELICS, getSkill } from "./data.js";
import { clearSave, hasSave, loadGame } from "./storage.js";
import { scoreLabel, scorePronunciation } from "./voice/scoring.js";
import { getPlatformLabel, vibrate } from "./platform.js";
  ...
  return `
    <section class="title-screen">
      ...
      <div class="title-actions">
        ...
        <p class="title-note">网页试玩会在本机自动存档。真机版需连接服务端粤语识别接口。</p>
      </div>
    </section>`;
}

function towerTemplate(state, engine) {
  const nextFloor = state.floor + 1;
  const lessonId = state.player.deck[(state.floor + 1) % state.player.deck.length];
  ...
  const gates = state.floorOptions.map((option) => `
...
function battleTemplate(state, engine) {
  const combat = state.combat;
  const enemy = combat.enemy;
  const intent = engine.getIntentPreview();
  const energy = Array.from({ length: 3 }, (_, index) => `<span class="energy-orb ${index >= combat.energy ? "spent" : ""}">${index < combat.energy ? "声" : "·"}</span>`).join("");
    ...
    <section class="battle-screen">
      <div class="battle-status">
        <div class="combatant-mini">
          <div class="label-line"><strong>你</strong><small>${state.player.hp}/${state.player.maxHp}</small></div>
          <div class="hp-meter"><span style="width:${percent(state.player.hp, state.player.maxHp)}%"></span></div>
        </div>
...
function shopTemplate(state) {
  const offers = state.shop.offers.map((offer) => {
...
function rewardTemplate(state) {
  const reward = state.reward;
  const cards = reward.choices.map((id) => skillCard(getSkill(id), { action: "reward-skill" })).join("");
  return `
...
function endTemplate(state, engine, victory) {
...
export class GameUI {
  constructor({ engine, voiceAdapter }) {
    this.engine = engine;
    ...
    this.platformLabel = document.querySelector("#platform-label");
    this.lastNotice = null;
    this.toastTimer = null;
    this.pendingVoice = null;
    this.bindEvents();
    this.platformLabel.textContent = getPlatformLabel();
  }
  ...
  render(state, options = {}) {
    ...
    if (!isTitle) {
      ...
      this.inventoryButton.querySelector("b").textContent = count;
    }
  ...
  openVoice(skill) {
    ...
    const slider = this.modalRoot.querySelector("#fallback-confidence");
    slider?.addEventListener("input", () => {
      this.modalRoot.querySelector("#confidence-value").textContent = `${slider.value}%`;
    });
  }
  ...
  startListening() {
    ...
    const orb = this.modalRoot.querySelector("#voice-orb");
    const text = this.modalRoot.querySelector("#voice-live-text");
    const startButton = this.modalRoot.querySelector('[data-action="start-listening"]');
  ...
  toggleFallback() {
    const panel = this.modalRoot.querySelector("#fallback-panel");
    if (!panel) return;
  ...
  submitFallback() {
    if (!this.pendingVoice) return;
    const transcript = this.modalRoot.querySelector("#fallback-transcript")?.value || "";
    const confidence = Number(this.modalRoot.querySelector("#fallback-confidence")?.value || 72) / 100;
  ...
  openHelp() {
    const adapterLabel = this.voiceAdapter.supported ? "当前环境可直接收音" : "当前环境使用键盘测试回退";
    this.modalRoot.innerHTML = `
      <div class="modal-sheet">
        ...
        <div class="notice-strip">真机接入时，录音由微信或抖音小游戏接口完成，音频上传到自有服务端，再调用粤语 ASR。密钥不可放在客户端。</div>
      </div>`;
  }

  closeModal(cancelVoice = true) {
