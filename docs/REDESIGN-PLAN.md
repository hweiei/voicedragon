...
> P6 进行中（动画/特效升级，方案见 `docs/FX-UPGRADE-PLAN.md`）：✅ F1 完成——FxDirector 演出编排（计划纯函数 + 调度器去抖/预算 + hit-stop 时钟门）替代旧版命令式 fx：命中闪光→分级抖动（振幅随伤害 2..8px）→浮字 0/16/40ms 交错、暴击（≥90 分）金光放大、View Transitions 场景转场（0KB，旧浏览器回退原 screen-in）、缓动令牌三件套、浮字对象池回收、reduce-motion 降级为纯透明度浮字；退役 .fx-shake/.fx-jolt/.fx-hurt 三组 CSS 改由 WAAPI 播放；174 单测（+24）+ 6 E2E（+2）全绿，预算 61.0/350 KB。待办：F2 声之形（语音光环 + 粒子 v2 + 三幕氛围 + 音频驱动）、F3 治理收尾
