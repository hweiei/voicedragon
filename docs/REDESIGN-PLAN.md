...
> P8-D ✅ 完成（学习闭环）：纯本地 SRS 新增 90 日每日聚合、3 个不同短句目标、连续练习与 14 日趋势；标题页和学习报告接入空档/变化展示；版本化学习档案支持白名单 JSON 导出、预览后二次确认覆盖恢复和 1 MiB 门限。旧 `voice-tower-srs-v1` 自动补历史；不保存或导出录音、识别文本、F0 帧，不改变游戏存档、设置、战斗评分与数值。356 项测试 + 32 E2E 全绿，预算 84.5/350 KB；P8-B 三幕参考胜率仍为 56.7% / 56.3% / 55.3%、零超时。详见 `docs/LEARNING-LOOP-PLAN.md`。
> P8-E ✅ 完成（发布与设备验收自动化）：新增 44 项 PWA 构建产物契约，默认相对路径与 GitHub Pages 子路径双检；Cloudflare CSP/HSTS/权限与缓存头收口，SW 注册器 defer；Chromium 桌面/触屏、Firefox、WebKit 四项目发布矩阵覆盖窄屏、CSP、异常档案与离线壳，共 19 通过、1 个 WebKit 工具限制明确跳过。移动/桌面 Lighthouse 变为硬预算，最近为 99/100/100/100 与 100/100/100/100。原 356 项测试 + 32 E2E 保持，预算 84.5/350 KB，P8-B 平衡不变。Safari/Android 真机、线上安全头、模型移动网络续传与分享抓图仍待部署后人工验收；详见 `docs/RELEASE-READINESS-PLAN.md`、`docs/DEVICE-TEST-MATRIX.md`。
> P9 ✅ 完成（守势反击）：`counterVersion:1` 独立版本（startCampaign 第六参数，组合根完整转发），新卡「还返俾你」仅经 `skillsFor(act,"p7",1)` 进新战役卡池；还击纯规则 `src/core/counter.ts` 预测与实际共用——被挡总量按 50% 还击（下限 1、露隙 ×1.25、被敌甲吸收），穿甲不触发不消耗、无伤害回合保留、guardAttack 先得甲再吃还击、可击杀可跨半血同回合提交二阶段。UI 姿态胶囊 + 还击预测行 + 流派文案按版本切换。375 项测试 + 35 E2E 全绿，发布矩阵 19 过 1 跳过、Lighthouse 99/100/100/100 与 100/100/100/100，预算 85.5/350 KB；P9 门三幕 59.3% / 63.3% / 53.0%、counterHits 1841/1925/1686，P8-B 曲线逐位不变。详见 `docs/COUNTER-ATTACK-PLAN.md`、`docs/P9-BALANCE-REPORT.md`。
> P10 ✅ 完成（名伶登场）：`rosterVersion:1` 独立版本 + `CampaignConfig` 参数对象重构（旧位置签名等价转发）。三名粤剧行当角色——文武生「亮相」（每场首次正音当次伤害+4）/花旦「绕梁」（调准≥80 每次+2护甲，无声通道不触发）/丑生「打诨」（破阵拍≥92 每回合回1气）；各带起始牌组与签名技（一夫当关/顾盼生辉/搞掂晒，角色限定池+升级入表，图鉴 40 招式）。名伶选择 sheet 含本地解锁条件（一幕 Boss/8 成就）。396 项测试 + 38 E2E 全绿，预算 87.6/350 KB；P10 门三角色三幕 63.0–52.0% 全带内、零超时；五条旧仿真门逐位零漂移。详见 `docs/ROSTER-PLAN.md`、`docs/P10-BALANCE-REPORT.md`、`docs/GROWTH-PLAN.md`（P11–P15 路线）。
> P11 ✅ 完成（声动九霄）：`ultimateVersion:1` 独立版本（CampaignConfig 直通）。满堂彩——裸分 ≥85 蓄彩 +1 封顶 3、<65 断、其间保持、每场绝技一次；彩满 3 发动角色绝技长句（完整语音评分、不耗声气、不回馈彩）：文武生「锣鼓响好戏开场」重击+正音无视护甲 / 花旦「莺声婉转绕梁三日」回复+护甲+清干扰 / 丑生「好戏在后头」虚弱+夺全甲+换手。绝技句进 ALL_SKILLS（43 招式，图鉴/练习场/SRS 可见）但不进任何卡池；战斗 BGM 纵向分层（节奏层=Boss 二阶段或血 <40%、彩层=彩 ≥2）+ 绝技/三星 stinger，全部振荡器合成零素材。416 项测试 + 41 E2E 全绿，发布矩阵 19 过 1 跳，Lighthouse 移动 99（TBT 112ms）/桌面 100×4，预算 89.9/350 KB；默认行=P10 基线逐位零漂移、高声韵行 5444 casts、六条旧仿真门数据行零漂移。彩规则经三轮 sim:p11 校准定案（最终分/严格连续两案均证伪）。详见 `docs/ULTIMATE-PLAN.md`、`docs/P11-BALANCE-REPORT.md`。
